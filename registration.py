...
def coarse_transform(dataset, img_size, img_t0, img_t1, gray_img_t0, gray_img_t1):
    ...
    image0 = torch.tensor(gray_img_t0, dtype=torch.float32, device=device) / 255.0
    image1 = torch.tensor(gray_img_t1, dtype=torch.float32, device=device) / 255.0
    ...
    # Find homography
    homography_matrix, mask = cv2.findHomography(pts_dirty, pts_clean, cv2.RANSAC, 5.0)

    # Warp dirty image to align with clean image
    aligned_dirty_image = cv2.warpPerspective(img_t1, homography_matrix, (gray_img_t0.shape[1], gray_img_t0.shape[0]))
    # aligned_dirty_image = cv2.warpPerspective(img_t1, homography_matrix, img_size)

    cv2.imwrite('aligned.jpg', aligned_dirty_image)
    ...
    if mask is not None:
        ...
        if inlier_ratio >= thr:
            flag = True
    print('Flag: ', flag)
    return aligned_dirty_image, homography_matrix, flag